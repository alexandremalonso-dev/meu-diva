...
export default function AdminFinancialReportPage() {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      setLoading(true);
      console.log("📥 [ADMIN] Carregando dashboard financeiro...");

      const [data, patientsResult, therapistsResult] = await Promise.allSettled([
        apiCall({ url: "/api/admin/reports/dashboard", requireAuth: true }),
  ...
  useEffect(() => {
    ...
    filtered.forEach(s => {
      ...
      // Apenas status que geram receita entram no gráfico
  ...
  const exportToCSV = () => {
    const headers = [
      ...
      "Valor (R$)", "Motivo", "Política Aplicada"
    ];
    
    const rows = filteredSessions.map(s => [
      s.appointment_id, s.date, s.time, s.patient_name, s.therapist_name,
      getStatusLabel(s.status),
      s.status === "cancelled_by_therapist" ? `-${formatCurrency(s.session_price)}` : formatCurrency(s.session_price),
    ...
    let filename = "relatorio_financeiro_admin";
    if (useCustomPeriod && customStartDate && customEndDate) {
        ...
        <div className="flex items-center justify-between flex-wrap gap-4">
          <div>
            ...
            <p className="text-gray-600 mt-1">
              Acompanhe o faturamento e gestão financeira de toda a plataforma
            </p>
          </div>
            ...
            <ReLineChart data={chartData} margin={{ top: 5, right: 30, left: 20, bottom: 5 }}>
              ...
              <Tooltip
                formatter={(value: number, name: string) => {
                  if (name === "Receita (R$)" || name === "Acumulado (R$)") return [formatCurrency(value), name];
                  return [value, name];
                }}
              ...
              <Legend />
              {chartMetric === "revenue" && (
                <Line type="monotone" dataKey="revenue" stroke="#E03673" strokeWidth={2} dot={{ fill: '#E03673', r: 4 }} name="Receita (R$)" />
              )}
                              ...
                              <img 
                                ...
                                onError={(e) => {
                                  ...
                                  if (e.currentTarget.parentElement) {
                                    ...
                                    e.currentTarget.parentElement.className = "w-8 h-8 rounded-full bg-gradient-to-br from-[#E03673] to-[#E03673]/80 flex items-center justify-center text-white text-xs font-bold flex-shrink-0";
                                  }
                                }}
                              />
                            ) : (
