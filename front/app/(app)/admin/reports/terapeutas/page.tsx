...
export default function AdminTerapeutasReportPage() {
  const { user } = useAuth();
  ...
  // 🔥 CARREGAR DADOS - ENDPOINT /terapeutas
  const loadData = useCallback(async () => {
    if (!user?.id) return;
    
    try {
      setLoading(true);
      console.log("📥 [ADMIN] Carregando relatório por terapeuta...");

      const [data, patientsResult, therapistsResult] = await Promise.allSettled([
        apiCall({ url: "/api/admin/reports/terapeutas", requireAuth: true }),
  ...
  const exportToCSV = () => {
    ...
    let filename = "relatorio_terapeutas";
    if (useCustomPeriod && customStartDate && customEndDate) {
        ...
        <div className="flex items-center justify-between flex-wrap gap-4">
          <div>
            ...
            <p className="text-gray-600 mt-1">
              Acompanhe o faturamento e gestão financeira dos terapeutas
            </p>
          </div>
        ...
        <div className="flex items-center justify-between mb-4 flex-wrap gap-3">
          ...
          <div className="flex gap-2">
            <button
              ...
              className={`px-3 py-1 text-sm rounded-lg transition-colors ${chartMetric === "revenue" ? 'bg-[#2F80D3] text-white' : 'bg-gray-100 text-gray-700'}`}
            >
              Receita Líquida (R$)
            </button>
            <button
              onClick={() => setChartMetric("sessions")}
              className={`px-3 py-1 text-sm rounded-lg transition-colors ${chartMetric === "sessions" ? 'bg-[#2F80D3] text-white' : 'bg-gray-100 text-gray-700'}`}
            >
              Sessões
            </button>
            <button
              onClick={() => setChartMetric("cumulative")}
              className={`px-3 py-1 text-sm rounded-lg transition-colors ${chartMetric === "cumulative" ? 'bg-[#2F80D3] text-white' : 'bg-gray-100 text-gray-700'}`}
            >
              Acumulado (R$)
            </button>
          </div>
        ...
        {chartData.length === 0 ? (
          ...
          <ResponsiveContainer width="100%" height={350}>
            <ReLineChart data={chartData} margin={{ top: 5, right: 30, left: 20, bottom: 5 }}>
              ...
              <YAxis tickFormatter={(value) => chartMetric === "sessions" ? value : `R$ ${value}`} tick={{ fontSize: 12 }} />
              <Tooltip formatter={(value: number, name: string) => {
                if (name === "Receita Líquida (R$)" || name === "Acumulado (R$)") return [formatCurrency(value), name];
                return [value, name];
              }} labelFormatter={(label) => `Dia ${label}`} />
              <Legend />
              {chartMetric === "revenue" && (
                <Line type="monotone" dataKey="revenue" stroke="#2F80D3" strokeWidth={2} dot={{ fill: '#2F80D3', r: 4 }} name="Receita Líquida (R$)" />
              )}
              {chartMetric === "sessions" && (
                <>
                  <Line type="monotone" dataKey="sessions" stroke="#10B981" strokeWidth={2} dot={{ fill: '#10B981', r: 4 }} name="Sessões (Agendadas + Realizadas)" />
              ...
              {chartMetric === "cumulative" && (
                <Line type="monotone" dataKey="cumulative" stroke="#2F80D3" strokeWidth={2} dot={{ fill: '#2F80D3', r: 4 }} name="Acumulado (R$)" />
              )}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-5">
          <div className="flex items-center justify-between mb-2">
            <span className="text-sm text-gray-500">Receita Líquida Total</span>
            <TrendingUp className="w-5 h-5 text-green-500" />
          </div>
          <p className="text-2xl font-bold text-gray-900">{formatCurrency(summary.total_revenue)}</p>
          <p className="text-sm text-gray-500 mt-1">{summary.total_completed_sessions + summary.total_pending_sessions} sessões totais</p>
        </div>
        
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-5">
          <div className="flex items-center justify-between mb-2">
            <span className="text-sm text-gray-500">Ticket Médio Líquido</span>
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-5">
          ...
          <p className="text-sm text-gray-500 mt-1">{formatCurrency(summary.total_pending_value)} a realizar</p>
        </div>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-5 mb-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 pt-4 border-t border-gray-100">
          <div>
            ...
            <select value={statusFilter} onChange={(e) => setStatusFilter(e.target.value)} className="w-full p-2 border border-gray-200 rounded-lg focus:ring-2 focus:ring-[#2F80D3] outline-none">
                    ...
                    <tr key={session.id} className="hover:bg-gray-50 transition-colors">
                      ...
                      <td className="p-3">
                        ...
                        {session.not_occurred_reason && (
                          <div className="text-xs text-gray-400 mt-1">{getReasonLabel(session.not_occurred_reason, session.status)}</div>
                        )}
                      </td>
                      <td className="p-3 text-right">
                        <span className={`text-sm font-medium ${session.status === "cancelled_by_therapist" ? 'text-red-600' : session.status === "completed" ? 'text-green-600' : 'text-gray-600'}`}>
                          {session.status === "cancelled_by_therapist" ? `-${formatCurrency(session.session_price)}` : formatCurrency(session.session_price)}
                        </span>
                      </td>
                      <td className="p-3">
                        <span className="text-xs text-gray-500">{getPolicyApplied(session.status, session.not_occurred_reason, session.has_medical_record)}</span>
                      </td>
                      <td className="p-3 text-center">
                        <button onClick={() => handleViewSession(session.id)} className="p-1.5 text-gray-400 hover:text-[#2F80D3] transition-colors" title="Ver prontuário">
