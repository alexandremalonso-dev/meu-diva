...
import { Calendar, User, Video, RefreshCw, XCircle, Clock, CheckCircle, Copy, Filter, ArrowUp, ArrowDown, MessageSquare, MapPin, ChevronDown, ChevronUp } from 'lucide-react';
...
interface SessionsListProps {
  ...
  onRegisterComplaint?: (appointment: Appointment) => void;
  userRole?: string;
}
...
export default function SessionsList({
  ...
  const handleStartSession = (apt: Appointment) => {
    if (apt.id) router.push(`/patient/videochamada/${apt.id}`);
    if (apt.id) openQueixa(apt.id);
  };
  ...
  if (sorted.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-16 text-center">
        ...
        <p className="text-sm text-gray-400 mt-1">Agende uma sessão para começar</p>
      </div>
    );
  }
    ...
    <div>
      ...
      <div className="flex items-center justify-between mb-4">
        <p className="text-sm text-gray-500">
          <span className="font-semibold text-gray-800">{sorted.length}</span> sessões agendadas
        </p>
        <button
        ...
        {Object.entries(grouped).map(([dateLabel, apts]) => (
          <div key={dateLabel}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-3">
              {apts.map(apt => {
                const therapist = apt.therapist;
                const therapistName = therapist?.full_name || `Terapeuta ${apt.therapist_user_id}`;
                const fotoUrl = getFotoUrl(therapist?.foto_url);
                ...
                const isFrequent = frequentTherapists.some(t => t.user_id === apt.therapist_user_id);
                    ...
                    <div className="p-4">
                      ...
                      <div className="flex items-start gap-3 mb-3">
                        <div className="w-10 h-10 rounded-xl overflow-hidden bg-gradient-to-br from-[#2F80D3]/20 to-[#E03673]/20 flex-shrink-0 flex items-center justify-center">
                          {fotoUrl
                            ? <img src={fotoUrl} alt={therapistName} className="w-full h-full object-cover" />
                            : <User className="w-5 h-5 text-[#2F80D3]" />}
                        </div>
                        <div className="flex-1 min-w-0">
                          <p className="font-semibold text-gray-900 text-sm truncate">{therapistName}</p>
                          <div className="flex items-center gap-1.5 mt-0.5">
                            <Clock className="w-3 h-3 text-gray-400" />
                            <span className="text-xs text-gray-500 font-medium">{time}</span>
                            {daysUntil && (
                              <span className="text-xs text-gray-400">· {daysUntil}</span>
                            )}
                          </div>
                        </div>
                      ...
                      {isConfirmed && (
                        <div className="flex gap-2 mb-2">
                          ...
                          <button
                            onClick={() => onRegisterComplaint?.(apt)}
                      ...
                      {/* Expandir para cancelar/reagendar */}
                      {isConfirmed && (
                        <button
                      ...
                      {isExpanded && isConfirmed && (
                        <div className="mt-2 pt-2 border-t border-gray-100 space-y-2">
                          ...
                          {showReschedule[apt.id] && !hasBeenRescheduled && (
                            <div className="space-y-2 pt-1">
                              <input
                                ...
                                className="w-full p-2 border border-gray-200 rounded-lg text-xs focus:ring-2 focus:ring-[#E03673] focus:border-transparent outline-none"
                          ...
                          {hasBeenRescheduled && (
                            <p className="text-xs text-orange-500 text-center">Esta sessão já foi reagendada</p>
                          )}
                        </div>
                      )}
                    </div>
