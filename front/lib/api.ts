...
function getApiBaseUrl(): string {
  ...
  if (
    ...
    host.includes("meudiva-frontend-prod")
  ) {
    return "https://api.meudivaonline.com";
  }
  if (
    host.includes("meudiva-frontend-non-prod") ||
...
export async function api(
  ...
  if (!res.ok) {
    const errorMessage =
      data?.error || data?.detail || `Erro ${res.status}`;
    const isSilent =
      SILENT_STATUSES.includes(res.status) || options?.silent;
...
// Exporta helper para uso em componentes que precisam da URL base
