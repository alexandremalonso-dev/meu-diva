...
export function ChatDrawerContent({ userRole }: ChatDrawerContentProps) {
  ...
  useEffect(() => {
    if (activeThreadId) setTimeout(() => scrollToBottom('instant'), 100);
  }, [activeThreadId]);
              ...
              >
                <div className="w-10 h-10 rounded-full overflow-hidden bg-gray-200 flex items-center justify-center flex-shrink-0">
                  {conv.other_user_foto_url
                    ? <img src={getFotoSrc(conv.other_user_foto_url)!} alt={conv.other_user_name} className="w-full h-full object-cover" />
  ...
  return (
    <div className="w-full flex flex-col" style={{ height: '100%', minHeight: 0 }}>
          ...
          <div className="flex items-center gap-2 flex-1 min-w-0">
            <div className="w-8 h-8 rounded-full overflow-hidden bg-gray-200 flex items-center justify-center flex-shrink-0">
              {activeConversation.other_user_foto_url
                ? <img src={getFotoSrc(activeConversation.other_user_foto_url)!} alt={activeConversation.other_user_name} className="w-full h-full object-cover" />
      ...
      {/* Mensagens — flex-1 com overflow-y-auto */}
      <div ref={messagesContainerRef} className="flex-1 overflow-y-auto space-y-2 px-1 pb-2" style={{ minHeight: 0 }}>
        {loading ? (
          <div className="flex justify-center py-8">
            <Loader2 className="w-6 h-6 text-[#E03673] animate-spin" />
          </div>
      ...
      <div className="border-t border-gray-100 pt-2 flex-shrink-0">
        {imageError && <p className="text-xs text-red-500 mb-1 text-center">{imageError}</p>}
        <ChatInput
